...
# To do: filter out ground, low vegetation points.  Speed up segmentation?  DLT_040 takes about 2m using li2012
...
#DLT_040
# 2. Define the center coordinates and radius
center_coords <- c(x = 47200, y = 23900)
radius <- 40 # The units depend on the CRS

# 3. Create an sf point object for the center
# This example uses a projected CRS (EPSG:3857) with meters as units.
# The `st_sfc()` function creates a simple feature geometry list-column.
center_point <- st_sfc(st_point(center_coords), crs = cs13_m)

# 4. Generate the circular polygon using st_buffer()
DLT_040 <- st_buffer(center_point, dist = radius)
aa <- DLT_040

# m <- mapview(DLT_040, basemaps = c("Esri.WorldImagery"))
# st_write(DLT_040, "DLT_040.kml")

# las_DLT_040 <- clip_roi(ctg, DLT_040)
# las <- las_DLT_040
...
# 
# filter_poi()#remove points from nlas where class is ground
nlas <- filter_poi(nlas, Z>3)#remove points from nlas where height is low
...
# 
# #8.2
# algo <- dalponte2016(chm, ttops) #Fast, but needs adjustment, max tree area too small
...
seg <- segment_trees(las = nlas, algorithm = li2012()) #10:20 pm 9/30
...
mapview(metrics, zcol = "Z", at = seq(40, 70))
...
seg_snags <- segment_snags(seg, wing2015(neigh_radii = c(1.5, 1, 2), BBPRthrsh_mat = bbpr_thresholds)) #8:51 pm start, done by 9:30

# Plot it all, tree and snag points...
# plot(seg_snags, color="snagCls", pal = rainbow(5))
# plot(seg_snags, color="snagCls")

# writeLAS(seg_snags, "LHH_aa_z3segssnags.las", index = FALSE)
# seg_snags <- readALSLAS("LHH_aa_z3segssnags.las") #assigns type = airborne
...
# # mapview(chm)+
# 
# aer2024 <- "https://maps.sfdpw.org/arcgis/rest/services/Pictometry/Pictometry2024/MapServer"
# tmap_mode("view") 
# 
# tmap_aer <- tm_shape(crown_outlines) + 
#   # Add the  aerial photo layer as the basemap
#   tm_basemap(server = aer2024)+
#   # Add the vector layer on top (with some transparency)
#   tm_polygons(border.col = "grey", fill = NA, lwd = 1)+
#   tm_polygons(snag_outlines, border.col = "black", fill = NA, lwd = 2)
# 
# # 6. Print the map
# tmap_aer

# #GB 10/5/2025 - works!
# # 1. Set tmap to interactive mode
# # Tile layers are only supported in "view" mode (powered by leaflet)
# tmap_mode("view") 
# 
# # 2. Define the custom Esri MapServer Tiled URL
# # This URL uses the XYZ tile pattern: {z}oom/{y}row/{x}column
...
#   tm_borders(col = "green", lwd = 0.5, alpha = 0.5) # +
#   # Set the initial map view to San Francisco (Lat: 37.77, Lon: -122.41)
#   # tm_view(lat = 37.77, lon = -122.41, zoom = 14, set.view = 10)
# 
...
tmap_sf_aerial <- tm_shape(crown_outlines) +
  ...
  #            group = "SF Pictometry 2024") +
  # Add a simple vector layer (country borders) on top for context
  tm_borders(border.col = "green", fill = NA, lwd = 0.5, alpha = 0) +
...
#/GB
  ...
  mapview(metrics, zcol = "Z")+#, at = seq(40, 70))+
  mapview(crown_outlines, fill = NA)+
  mapview(snag_outlines, fill = NA, color = "black", lwd = 2)
