...
if (USE_CUSTOM_CIRCLE) {
  message("Using DLT_040 test circle  (center = [",
          ...
          "],  r = ", CIRCLE_RADIUS, " m)")
  center_point <- st_sfc(
    st_point(c(x = CIRCLE_CENTER_X, y = CIRCLE_CENTER_Y)), #Dan's Lost Trail tree swing area
    st_point(c(x = 47600, y = 26390)), #Mclaren Lodge Cypress (CS13 ft) 156202.54, 86547.58
    # st_point(c(x = 50856, y = 27290)), #City Hall
    # st_point(c(x = 43915, y = 27950)), #ECDM rain garden
    crs = cs13_m
  )
